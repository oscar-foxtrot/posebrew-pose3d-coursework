...
# import scipy
"""
kpts0 = np.load('file_469_2.npy', allow_pickle=True)[100:-100]
kpts1 = np.load('file_474_2.npy', allow_pickle=True)[:]
...
window_size = 80
smooth_norms = pd.Series(norms).rolling(window=window_size, center=True, min_periods=1, win_type='gaussian').mean(std=window_size/4)

plt.plot(norms - smooth_norms, label='Евклидова норма разности векторов,\nвырезано 100 последних кадров из видео 469')
#plt.plot(smooth_norms, label='Евклидова норма разности векторов,\nвырезано 100 последних кадров из видео 469')
plt.legend()
plt.show()
...
print(np.mean(norms))
print(np.std(norms))
print(np.std(norms) / np.mean(norms))
"""
"""
...
# --- Load keypoints
kpts1 = np.load('file_469_2.npy', allow_pickle=True)[100:-100]
kpts0 = np.load('file_474_2.npy', allow_pickle=True)[:]
kpts0 = np.array(kpts0, dtype=np.float64)
kpts1 = np.array(kpts1, dtype=np.float64)
...
plt.xlabel('Смещение видео 1 относительно начала видео 2')
plt.plot(norms, label='Евклидова норма разности векторов,\nвырезано 100 последних кадров из видео 469', color='black')

#plt.plot(norms, label='Среднее евклидова расстояния\nмежду точками (между кадрами)')
#plt.legend()
plt.savefig('minimums.png', dpi=300)
plt.show()
