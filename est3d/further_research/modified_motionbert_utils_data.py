# lib/utils_utils_data.py file from MotionBERT modified so as to normalize at each window individually
...
def crop_scale(motion, scale_range=[1, 1], vid_size=(1920, 1080)):
    '''
        Motion: [(M), T, 17, 3].
        Normalize to [-1, 1]
    '''
    ...
    xmin = 0 #min(valid_coords[:,0])
    xmax = w #max(valid_coords[:,0])
    ymin = 0 #min(valid_coords[:,1])
    ymax = h #max(valid_coords[:,1])
