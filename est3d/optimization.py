...
plt.plot(np.array(skeleton_sums) / np.mean(skeleton_sums), label='file_795, позвоночник, без ограничения на тренд', alpha=0.4, color='red')
ss0 = skeleton_sums
...
for i in range(len(keypoints)):
    skeleton_sums += [get_ss_weighted(h36m_pts, keypoints[i], w0_floats)]
plt.plot(np.array(skeleton_sums) / np.mean(skeleton_sums), label='file_795, позвоночник, с ограничением на тренд', alpha=0.4, color='blue')
ss1 = skeleton_sums
...
for i in range(len(keypoints)):
    skeleton_sums += [get_ss_weighted(h36m_pts, keypoints[i], w0_floats)]
plt.plot(np.array(skeleton_sums) / np.mean(skeleton_sums), label='file_795, голень, без ограничения на тренд', alpha=0.4, color='green')
ss2 = np.array(skeleton_sums) * 0.66
...
for i in range(len(keypoints)):
    skeleton_sums += [get_ss_weighted(h36m_pts, keypoints[i], w0_floats)]
plt.plot(np.array(skeleton_sums)  / np.mean(skeleton_sums), label='file_795, сумма всех костей', alpha=0.4, color='orange')
ss3 = np.array(skeleton_sums) * 1.08
