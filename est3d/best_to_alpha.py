...
res = []
for frame in rtm:
    if id not in frame['instances']:
        continue
    a = dict()
    a['image_id'] = f"{frame['frame_id']}"
    a['category_id'] = 0
    a['keypoints'] = []
    for k in range(26):
        a['keypoints'] += frame['instances'][id]['keypoints'][k]
        a['keypoints'] += [frame['instances'][id]['keypoint_scores'][k]]
    a['box'] = frame['instances'][id]['bbox'][0]  # ind
    a['idx'] = [0.0]
    res += [a]
...
with open(f'output_file{file_number}/predictions/file_{file_number}_toalpha_0.json', 'w') as f:
    json.dump(res, f, indent=4)

with open(f'output_file{file_number}/predictions/file_{file_number}_toalpha_1.json', 'w') as f:
    json.dump(res1, f, indent=4)

with open(f'output_file{file_number}/predictions/file_{file_number}_toalpha_2.json', 'w') as f:
