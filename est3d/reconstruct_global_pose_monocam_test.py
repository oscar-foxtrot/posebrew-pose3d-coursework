...
def estimate_global_f(
    ...
    for t in range(T):
        ...
        S = 1 / skeleton_sums[t]
        #S = 1 / (skeleton_sums[t] / np.mean(skeleton_sums[t])) * 10000000
...
skeleton_sums = np.asarray(skeleton_sums)[range_frames]
...
def pixel_to_ray(u, v, f, cx, cy):
    x = (u - cx) / f
    y = (v - cy) / f
    ray = np.array([x, y, 1.0])
    return ray / np.linalg.norm(ray)

# camera origin in YOUR coordinate convention
C = np.array([0.0, 0.0, 0.0])

# scale just for visualization (how far to draw rays)
L = 2.0

rays = []
for (u, v) in img_corners:
    r = pixel_to_ray(u, v, f, cx, cy)
    rays.append(r)
...
#ani.save("animations/file_102_monocular_world.gif", writer="ffmpeg", fps=30)
# Show the plot
plt.show()


#np.save(f"temp_seq/file_{file_number}_reproj_3d.npy", keypoints_3d)
